...
use nostr_archive_cursor::{DEFAULT_FRAME_TARGET, FrameTable, convert_archive_to_zst, sidecar_path};
...
fn main() -> anyhow::Result<()> {
    ...
    let limit: usize = std::env::args().nth(2).and_then(|a| a.parse().ok()).unwrap_or(200_000_000);
    ...
    let n = {
        ...
        let mut got = 0;
        while got < limit {
            match r.read(&mut raw[got..])? { 0 => break, k => got += k }
        }
        got
    };
    raw.truncate(raw[..n].iter().rposition(|&b| b == b'\n').map(|p| p + 1).unwrap_or(n));
    match src.extension().and_then(|e| e.to_str()) {
        Some("gz") => { let mut e = flate2::write::GzEncoder::new(std::fs::File::create(&local)?, Default::default()); e.write_all(&raw)?; e.finish()?; }
        Some("bz2") => { let mut e = bzip2::write::BzEncoder::new(std::fs::File::create(&local)?, bzip2::Compression::default()); e.write_all(&raw)?; e.finish()?; }
        Some("zst") | Some("zstd") => std::fs::write(&local, zstd::encode_all(&raw[..], 3)?)?,
        _ => std::fs::write(&local, &raw)?,
    }
    println!("source {name}: {:.1} MiB raw -> local copy {:.1} MiB",
    ...
    println!("OK - {} bytes byte-identical after conversion", decoded.len());
